...
Публикация настроена и **ещё ни разу не запускалась** (M-93). Координаты —
`io.github.youndie.booblik`, репозиторий — reposilite snapshots, адрес и креды приходят из
`BOOBLIK_REPO_URL` / `_USER` / `_SECRET` и в репозитории не лежат. Запуск ручной:
