...
gh workflow run central.yaml -R youndie/sborka \
  -f repository=youndie/booblik -f ref=<тег> -f version=<X.Y.Z> -f runner=macos-15
```

`runner=macos-15` здесь не опция: в сборке есть Apple-таргеты, а KMP-модуль публикует корень и все
таргеты **с одного хоста** — иначе корень ссылается на варианты, которых никто не собрал.
