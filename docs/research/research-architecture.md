...
Но, в отличие от §1.9, подтвердить это исходниками нечем, а стек там — ext4 **на виртуальном
диске поверх NTFS** (§1.12). Поведение writeback — ровно то, что этот стек и меняет: под ним
ещё один кэш, чужой планировщик и чужая ФС. Поэтому честная формулировка такая: **в WSL2
не воспроизвелось; на bare-metal Linux не проверено**. Риск 1 остаётся открытым, а не снятым.
