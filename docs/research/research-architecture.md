...
**Поправка, найденная при перепрогоне на Linux (замер 10).** Всё выше — **свойство APFS**,
а не `msync`. На ext4 хвостовой `fsync` после `msync` стоит **0,21 мс = 2 %** от голого: там
`msync` работу уже сделал.

| Хост | `fsync` | `msync` | хвостовой `fsync` | вывод |
|---|---:|---:|---:|---|
| macOS / APFS | 8,81 мс | 13,96 мс | 6,95 мс = **79 %** | не барьер |
| Linux / ext4 | 11,01 мс | 17,14 мс | 0,21 мс = **2 %** | барьер |

Решение кода от этого только укрепилось: звать оба вызова обязательно на APFS и почти бесплатно
на ext4, то есть это портируемый вариант. А вот формулировка риска 4 («вывод переносится, проценты
нет») оказалась слишком мягкой: **не переносится и вывод**. Переносится только метод — спросить
у `fsync`, осталась ли работа.
...
**Поправка (замер 10).** Качели — тоже **macOS**. На ext4 маппинг не гуляет вовсе:
1,69 млн записей в секунду при разбросе **1,1×** и худшей секунде 1,61 млн, против 1,30 млн
у `FileChannel`. То есть на платформе, где брокеры живут, риск 1 не подтверждается, а маппинг
просто на 30 % быстрее и так же ровен.
