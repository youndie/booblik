...
**Первое число (M0, [benchmarking](../benchmarking.md), замер 1).** Маппинг быстрее в 41 раз
на записи по одной штуке: 13,9 млн записей в секунду против 336 тысяч при 64 байтах. То есть
драфт в этом пункте оказался прав, а Kafka делает иначе не потому, что mmap медленнее.

Но сравнение **пока нечестное**, и это видно, если перевести в наносекунды: 3,0 мкс у
`FileChannel` — это цена одного `write`, 72 нс у маппинга — цена `memcpy`. Мы сравниваем
...
оказался в 63 раза дешевле `fsync`, и число, меряющее более слабую гарантию, нельзя класть
