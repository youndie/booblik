...
### Сценарий: ёмкость сегмента понижена под существующим логом
* **Дано:** партиция, записанная с `segment.capacity.bytes` = 512 МиБ, и в ней данных больше,
  чем новая ёмкость.
* **Когда:** брокер стартует с меньшей `segment.capacity.bytes`.
* **Тогда:** **отказ на старте** с именем файла, размером на диске и запрошенной ёмкостью. Ничего
  не усечено, ничего не записано; со старой ёмкостью лог открывается целиком.
* **Почему отказ, а не предупреждение:** восстановление берёт предел из **ёмкости**, поэтому
  меньшая останавливала его раньше конца данных, а `truncateTo` сразу за ним ставил нулевой префикс
  длины — записи оставались на диске и становились недостижимыми. Предупреждение на старте
  конкурирует со строками пропускной способности, а ноль к моменту прочтения уже записан.
* **Ценой чего:** отказ срабатывает и когда данных мало, а файл `MAPPED` просто преаллоцирован под
  бо́льшую ёмкость. Это сознательно: у преаллоцированного файла длина не говорит о содержимом,
  и отличить «влезет» от «не влезет» без полного восстановления нельзя.
* **Автоматизирован:** `SegmentCapacityChangeTest` — отказ, неизменность файла при отказе,
  и что та же и бо́льшая ёмкость по-прежнему открываются.
