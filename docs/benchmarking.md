...
Умолчание **не менялось этим прогоном**: производительность больше не аргумент в его пользу,
но заменять довод о корректности на довод о скорости было бы ровно той ошибкой, из-за которой
`msync` полгода считался барьером. Решение вынесено в **M-45**, после M-60.
