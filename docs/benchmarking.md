...
**Итоговый образ — `bellsoft/liberica-openjre-alpine:25`, 176 МБ против 364 у temurin на Debian**
(числа из 18.2; здесь изначально стояла пара 240/514, снятая на WSL). Размер уменьшился в 2,1 раза,
...
### 18.2 Перемер размеров: отношение уцелело, абсолютные числа — нет
...
**Отношение 2,1× подтвердилось, абсолютные числа отозваны.** Все три записанных значения (240,
514, 197) оказались примерно на 55–150 МБ больше того, что даёт база плюс дистрибутив; общего
объяснения у расхождения нет, а гадать о хосте, которого больше нет, — ровно то, что запрещает
правило 3. Записаны числа, снятые здесь и проверяемые одной командой.
