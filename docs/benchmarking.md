...
## Замер 26. Сто часов на стенде и цена возраста лога (M-162)

**Ряды лежат в [`measurements/soak-2026-09/`](measurements/soak-2026-09/)** вместе со скриптами,
которыми сняты. Стенд снесён 11.09.2026 — прогон можно повторить, эти ряды восстановить нельзя,
поэтому всё, на чём стоят числа ниже, перенесено в репозиторий.

Соак заводился не «погонять подольше», а против четырёх утверждений, каждое из которых надо было
попытаться опровергнуть. Три опровергнуть не удалось, четвёртое оказалось верным не всегда — и
именно оно интереснее остальных.

**Стенд:** fornex, 4 ядра, 5,9 ГиБ, ext4 на **loop-образе в 2 ГиБ** (`/mnt/booblik-soak`). Размер
образа фиксирован не ради удобства: рядом на коробке k0s, и забить ей диск нельзя ни при каком
свободном месте. Брокер `ghcr.io/youndie/booblik:0.3.0`, `restart=no` — смерть должна быть видна,
а не залечена. Топик `soak:2`, сегмент 16 МиБ, `retention.bytes` 256 МиБ **на партицию**. Нагрузка —
опубликованный с PyPI клиент, 50 записей/с по 1 КиБ; потребитель читает только партицию 0, поэтому
`consumed ≈ produced / 2` — это форма стенда, а не потеря.

Окно: 2026-08-31T19:52:11Z — 2026-09-04T23:51:44Z, **359 973 с (99,99 ч)**, 5983 выборки раз в
минуту, ни одного пропуска.

### 26.1 Что показали сто часов

| | итог |
|---|---|
| записей | 17 215 960 принято, 8 555 904 прочитано |
| `lag`, `resets`, ошибки продюсера и потребителя | 0 во всех 5983 строках |
| перезапусков, OOM | 0 |
| `df` | 2 808 → **525 016** КБ, вышел на бюджет и стоял |
| сегментов | 2 → **34**, устойчиво 32 |
| дескрипторов | 22 → **82**, устойчиво 82 (±2 на смене сегмента) |
| потоков | **17** от первой строки до последней |

**Единственная ненулевая строка — нулевая секунда**, где метрики ещё не отвечали и записались `-1`.

### 26.2 Память: прямая, которую нельзя было продлевать

`RssAnon` первые сутки рос **линейно, 2,1 МБ/ч, разброс шага 7 %** — форма, на которую прогрев
не похож, потому что прогрев замедляется. Проекция на сто часов давала +210 МБ.

Прямая была настоящей, вывод — нет. Величина встала на **126,8 МБ** и с 21-го часа не двигалась.
Разбивка по анонимным отображениям (`pmap -X` с хоста, раз в 5 минут) назвала виновника одним
диффом: за восемь часов сдвинулся **ровно один регион из двенадцати** — куча, `59 172 → 63 796 КБ`
из 65 536. Страницы трогались до потолка `-Xmx64M`, и на потолке кончились.

**Скорость, снятая внутри ограничения, описывает подход к ограничению, а не тенденцию.** Сумма
умела сказать только «растёт»; чтобы сказать «что», нужна разбивка по месту, а не проекция по
времени.

Сэмплер регионов заведён **отдельным файлом**, а не колонкой в работающем `sample.sh`: правка
скрипта, который прямо сейчас исполняется, портит его. И снимался с хоста, а не `docker exec` —
вторая JVM в cgroup брокера убивает наблюдаемого.

### 26.3 `errors 0` — правда сто часов и ложь в одном режиме

Последним актом том забивался намеренно (`fill.sh`), и здесь единственное утверждение из четырёх
не устояло.

**0.3.0 на полном томе:** продюсер не получает ответа вовсе (`TimeoutError` на нулевой записи),
чтение живо (1016 записей), а метрика показывает `produce 0/s | backlog 1 | errors 0` при
`health: healthy`. Ошибок ноль потому, что никто не назвал их ошибками.

**0.3.1 на том же томе, том же логе, той же секунде — меняется только бинарь:** отказ кодом
`PARTITION_UNAVAILABLE` сразу, на нулевой записи; чтение живо.

Два следствия, каждое дороже самого замера:

* **Освободить место недостаточно.** После удаления балласта партиция продолжала отказывать —
  и сразу, и после retention-проверки. Писать снова получилось только после рестарта брокера.
* **Клиент падал на правильном ответе.** Опубликованный `booblik==0.1.0` умирал с
  `ValueError: 6 is not a valid Code` — код на проводе был верен, разбор в клиенте не пережил
  кода, которого не знал. Починено в `0.1.1`, проверено на стенде обеими версиями подряд.

### 26.4 Цена возраста: восстановление против числа сегментов

Утверждение «восстановление не дорожает с возрастом» проверяется не одним числом, поэтому — кривая
из трёх точек. Один образ (`0.3.1`), одни настройки, одна коробка, **одна сессия**, варианты
**чередуются** (не пять одного, потом пять другого — иначе дрейф стенда достаётся тому, кто шёл
вторым). Ось — от `docker run` до строки `booblik listening`, то есть вместе со стартом контейнера
и JVM: обе части платятся одинаково всеми вариантами, поэтому **разница** между строками и есть
цена лога.

Страницы вытесняются **пофайлово** через `posix_fadvise(DONTNEED)`, а не сбросом кэшей всей
машины: рядом k0s, и глобальный сброс мерил бы соседей. Без вытеснения кэш грел бы ровно тот
вариант, который под подозрением.

| лог | сегментов | медиана | min | max |
|---|---:|---:|---:|---:|
| пустой том | 0 | **706 мс** | 646 | 783 |
| половина (retention урезал копию) | 16 | **960 мс** | 869 | 1577 |
| стосуточный | 32 (512 МиБ) | **1541 мс** | 1160 | 2089 |

Пять прогонов на вариант. **Цена идёт за сегментами, а не за часами:** 835 мс между пустым логом
и полным бюджетом retention, около 26 мс на сегмент. Возраст сам по себе не накапливается —
retention держит 32 сегмента, и брокер после ста часов поднимается ровно как после трёх при том же
бюджете. Утверждение устояло, и теперь у него есть наклон, а не заверение.

Половинный лог получен **не удалением файлов руками**: копия стосуточного лога отдана брокеру
с половинным бюджетом, и сегменты удалила retention. Оригинал не тронут.

**Разброс у логов шире, чем у пустого тома, и первый прогон каждой серии — самый долгий**
(2089 и 1577 против медиан). Это прогрев, а не свойство лога; поэтому в таблице стоят все пять
прогонов, а не «лучший».

### 26.5 Поправка про инструмент

По дороге сюда я записал было находку «брокер с недоступной на запись директорией данных
печатает конфигурацию и не делает ничего». Находки нет. Проверка отдельным прогоном: брокер
**падает за 10 секунд** с `exit=1` и стектрейсом из `PartitionLog.open`. Не увидел я его потому,
что мой измеритель читал у контейнера только stdout, а трейс уходит в stderr, и не проверял, не
завершился ли контейнер, — то есть ждал бы «listening» все 120 секунд от любого падения. Ошибка
была в стенде, и заметить её стоило одного прогона.
